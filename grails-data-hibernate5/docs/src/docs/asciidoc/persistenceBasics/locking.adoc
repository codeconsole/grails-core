...
The instance must be attached to the current session. `airport.refresh()` is unchanged, and `airport.refresh([:])` or `airport.refresh(lock: false)` behave exactly like it: a plain refresh with no lock and no transaction requirement. To lock by identifier and reload at the same time, use `Airport.lock(id, refresh: true)` as described below.

NOTE: There is no `airport.lock(refresh: true)`. Groovy resolves that call to the static `lock(Serializable id)` method with the options map as the identifier. The instance form is `airport.refresh(lock: true)`.
