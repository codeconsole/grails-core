...
The instance must be attached to the current session. `book.refresh()` is unchanged, and `book.refresh([:])` or `book.refresh(lock: false)` behave exactly like it: a plain refresh with no lock and no transaction requirement. To lock by identifier and reload at the same time, use `Book.lock(id, refresh: true)` as described above.

NOTE: There is no `book.lock(refresh: true)`. Groovy resolves that call to the static `lock(Serializable id)` method with the options map as the identifier. The instance form is `book.refresh(lock: true)`.
