...
By default GORM creates and reconciles every index declared in a mapping block when the datastore starts. Set `buildIndexes` to `false` to switch that off:
...
The setting applies to every connection when declared at the top level, and can be set per connection inside a `connections` block. It also suppresses index creation for domain classes registered after startup.
...
"Applied" means the server accepted the declaration, not that it had to build anything: `createIndex` is idempotent, so on every restart after the first most declarations are accepted immediately because the index is already there. If any declaration failed, the summary is logged at `WARN` instead and reports how many; the failures themselves are logged individually as they happen.
