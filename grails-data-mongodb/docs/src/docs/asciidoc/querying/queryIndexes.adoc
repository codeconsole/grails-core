...
Startup then continues without waiting. The indexes are still built one at a time per connection, on a daemon thread named `gorm-mongo-index-build-<connection>`. Different connections can build indexes concurrently, including when they use the same MongoDB server. The thread is released after the build finishes and it has been idle for one second.
