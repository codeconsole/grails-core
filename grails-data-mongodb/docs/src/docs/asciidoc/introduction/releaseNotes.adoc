...
* `grails.gorm` and `grails.mongodb` settings are now applied when GORM is given an existing `MongoClient`, such as the one contributed by Spring Boot. Configuration that was previously ignored on this path — `failOnError`, the default mapping and constraints, flush behaviour and multi-tenancy — now takes effect. See <<upgradeNotes,Upgrade Notes>> before upgrading an application that supplies its own client.
...
* The same holds inside a session or transaction opened through a connection, such as `Book.moreBooks.withTransaction { }`: `book.save()` in it was written to the default connection
* An application can be checkpointed and restored with CRaC: GORM closes every `MongoClient` it created, on every connection, before the checkpoint and builds replacements after the restore. See <<advancedConfig,Advanced Configuration>>.
