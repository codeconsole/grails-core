...
* An application can be checkpointed and restored with CRaC: GORM closes every `MongoClient` it created, on every connection and including the one built from Spring Boot's settings, before the checkpoint and builds replacements after the restore. See <<advancedConfig,Advanced Configuration>>.
