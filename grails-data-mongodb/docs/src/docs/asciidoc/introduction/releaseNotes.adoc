...
* `grails.mongodb` settings are now applied when GORM is given an existing `MongoClient`, such as the one contributed by Spring Boot. This also activates configured multi-tenancy (`grails.gorm.multiTenancy` or `grails.mongodb.multiTenancy`), which was previously ignored on this path. Applications using a supplied client must review their tenant resolver and tenant data: tenant discrimination and routing now follow the configured mode instead of `NONE`.
