...
GORM takes part in the Spring lifecycle, so an application can be checkpointed with https://openjdk.org/projects/crac/[CRaC] and restored. CRaC refuses to checkpoint a process holding open sockets, and a connected `MongoClient` holds several, so before the checkpoint GORM closes the client of every connection: the default one, each one under `grails.mongodb.connections`, and any added at runtime. When the application is restored, GORM builds a replacement for each from the same settings. An <<embeddedMongo,embedded MongoDB>> is stopped after the clients and started again before them.

Everything that reaches MongoDB through GORM uses the replacements, including multi-tenancy and the <<springDataInterop,Spring Data MongoDB integration>>. A `MongoClient` that application code obtained before the checkpoint, for example from `MongoDatastore.getMongoClient()`, is one of the closed ones: obtain it again after the restore rather than keeping it.

A client the application supplied to GORM is neither closed nor replaced. It belongs to whoever created it, and that code has to close it before the checkpoint for the checkpoint to succeed. The client that Spring Boot's settings produce, which GORM builds itself when the application declares no `MongoClient` bean of its own, is GORM's and is closed and built again like the rest.
