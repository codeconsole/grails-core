...
GORM creates and reconciles the indexes declared in your mapping blocks each time the datastore starts. Set `grails.mongodb.buildIndexes = false` to leave the indexes on the server untouched instead — see <<queryIndexes,Querying Indexing>> for details and when to use it.
