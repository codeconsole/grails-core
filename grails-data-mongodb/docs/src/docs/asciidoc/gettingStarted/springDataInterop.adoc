...
* a `MongoDatabaseFactory` bound to GORM's `MongoClient` and default database (GORM keeps ownership of the client; the factory will not close it, and uses the replacement GORM builds after a <<advancedConfig,CRaC restore>>),
...
NOTE: On the unified manager, `readOnly` only governs GORM's flush. A GORM write queued inside a read-only transaction is not flushed (see <<readOnlyTransactions,Read-Only Transactions>>), but a Spring Data write such as `mongoTemplate.insert(...)` goes directly into the shared `ClientSession` and still commits. A read-only transaction does not prevent Spring Data writes.
