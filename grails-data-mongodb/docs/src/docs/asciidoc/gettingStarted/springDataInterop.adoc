...
NOTE: On the unified manager, `readOnly` only governs GORM's flush. A GORM write queued inside a read-only transaction is not flushed (see <<transactions,Read-Only Transactions>>), but a Spring Data write such as `mongoTemplate.insert(...)` goes directly into the shared `ClientSession` and still commits. A read-only transaction does not prevent Spring Data writes.
