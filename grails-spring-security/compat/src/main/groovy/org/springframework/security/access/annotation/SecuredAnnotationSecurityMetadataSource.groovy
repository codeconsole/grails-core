...
class SecuredAnnotationSecurityMetadataSource extends AbstractFallbackMethodSecurityMetadataSource {
    ...
    private static Collection<ConfigAttribute> processAnnotation(Secured secured) {
        secured == null ? null : secured.value().collect { String token -> new SecurityConfig(token) } as List<ConfigAttribute>
    }
}
