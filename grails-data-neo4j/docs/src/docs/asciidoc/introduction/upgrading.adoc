...
Inside a `withConnection` block, a static method called on the class, such as `Book.list()`, and an instance method such as `book.save()` now use the block's connection. Previously they used the default connection, and only the methods called without naming the class, such as `list()`, used the block's. The same now holds inside a session or transaction opened through a connection, such as `Book.moreBooks.withTransaction { }`. Code that relied on reaching the default connection from inside such a block this way should name it, as in ``Book.'default'.list()``.
