...
* A connection declared under `grails.neo4j.connections` can be used: a datastore with one no longer fails to start, and an entity saved or deleted through it, as with `Book.moreBooks.save(book)`, reaches that connection rather than the default one
