...
* The same holds inside a session or transaction opened through a connection, such as `Book.moreBooks.withTransaction { }`, where `book.save()` went to the default connection
