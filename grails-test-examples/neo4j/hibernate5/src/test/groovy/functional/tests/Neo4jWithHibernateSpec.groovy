...
class Neo4jWithHibernateSpec extends Specification implements GrailsUnitTest {
    ...
    @PendingFeature(reason = "hasHibernatePlugin()'s manager.allPlugins.any { plugin.name ==~ /hibernate\\d*/ } " +
            "check does not detect a plugin registered via MockGrailsPluginManager#registerMockPlugin the way " +
            "this test expects, so secondaryDatastore never becomes true and the two mapping contexts collapse " +
            "into one alias as if no Hibernate plugin were present. Pre-existing gap in this example app's own " +
            "test, unrelated to the monorepo migration; needs a real MockGrailsPluginManager/GrailsPlugin " +
    ...
    void "test that both grailsDomainClassMappingContext and neo4jMappingContext are same when using Neo4j with Hibernate"() {
        ...
        hibernate.getPluginClass() >> Neo4jGrailsPlugin
