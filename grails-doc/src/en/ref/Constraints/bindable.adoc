...
Properties are not eligible for mass data binding by default.  Existing `bindable: true` declarations remain valid and continue to bind exactly those properties without any configuration change.  Code that previously relied on all typed properties being bindable without an allowlist must now declare `bindable: true`, supply an explicit `include` list to `bindData`, or use `@BindAllowed` on a controller action command object parameter.

Applications that require the previous permissive behavior may set `grails.databinding.legacyBindableDefault=true`.  In legacy mode, statically typed instance properties are bindable unless they are marked with `bindable: false`, are transient fields, are dynamically typed properties, or are static properties.
