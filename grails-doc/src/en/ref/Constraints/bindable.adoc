...
Applications can opt into deny-by-default binding allowlists by setting `grails.databinding.legacyBindableDefault=false`.  In secure mode, only properties declared with `bindable: true`, supplied in an explicit `include` list to `bindData`, or named by `@BindAllowed` on a controller action command object parameter are bound.  Properties marked `bindable: false` remain non-bindable in either mode.
