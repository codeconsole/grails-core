...
The `find` method allows querying with Hibernate's query language  link:{hibernate5GuideFromRef}index.html#hql[HQL] and querying by example. The basic syntax is:
...
* `queryParams` - A `Map` of query parameters such as `cache`, `readOnly`, `fetchSize`, `timeout`, `flushMode`, and `lock`
