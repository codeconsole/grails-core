...
`lock(id)` and `lock(id, refresh: false)` keep the behaviour they have always had, which differs by
implementation: GORM for Hibernate 7 rejects the call without a transaction, while GORM for Hibernate 5 returns
the instance and acquires no lasting lock. Always lock inside a transaction.
