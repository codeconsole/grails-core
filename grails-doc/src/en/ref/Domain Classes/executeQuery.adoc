...
Executes  link:{hibernate5GuideFromRef}index.html#hql[HQL queries]
...
* `metaParams` - A `Map` of pagination parameters `max` and/or `offset`, as well as Hibernate query parameters `readOnly`, `fetchSize`, `timeout`, `flushMode`, `cache`, and `lock`
