...
If no `include` list is supplied, `bindData` uses the target class binding allowlist.  Existing `bindable: true` declarations and explicit `include` lists continue to bind exactly the properties they name without configuration changes.  In Grails 8, a class with no allowlisted properties binds none of its request parameters by default.  An empty `include` list also binds no properties.
...
See the link:{constraintsRefFromRef}bindable.html[bindable] constraint documentation for more information on controlling default bindability.  To permit a property, declare it with `bindable: true`, add it to an explicit `include` list, or use `@BindAllowed` on a controller action command object parameter.  Applications may set `grails.databinding.legacyBindableDefault=true` to restore permissive legacy binding for the whole application.
