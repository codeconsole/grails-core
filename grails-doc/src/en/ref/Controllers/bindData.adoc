...
Only boolean values and the strings `'true'` and `'false'` are recognised for `grails.databinding.legacyBindableDefault`.  String matching ignores case and surrounding whitespace.  An unrecognised value logs a warning and enables secure binding.
