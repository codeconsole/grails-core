...
String values can be encoded with a specific source character set by passing a `java.nio.charset.Charset` to `encodeAsBase64`. The Base64 text itself is returned as UTF-8/ASCII-compatible text. `decodeBase64` takes no character set, because Base64 text is always ASCII; it returns the decoded bytes for the caller to interpret.
...
`decodeHex` decodes an empty string to an empty byte array. For any other value that is false by Groovy truth, such as `null`, `0` or an empty collection, it returns `null`.
...
The hexadecimal digest codecs (`encodeAsMD5`, `encodeAsSHA1` and `encodeAsSHA256`) compute their byte digest directly rather than calling the matching `Bytes` codec through the metaclass. Replacing `encodeAsMD5Bytes`, `encodeAsSHA1Bytes` or `encodeAsSHA256Bytes` at runtime therefore no longer changes what the hexadecimal codecs return.
