...
==== authoritiesToRoles()
Extracts role names from an array or `Collection` of {springSecurityApiDocsUrl}org/springframework/security/core/GrantedAuthority.html[GrantedAuthority].
...
Splits a comma-delimited String containing role names into a `List` of {springSecurityApiDocsUrl}org/springframework/security/core/GrantedAuthority.html[GrantedAuthority].
...
==== registerProvider()
Used by dependent plugins to register an {springSecurityApiDocsUrl}org/springframework/security/authentication/AuthenticationProvider.html[AuthenticationProvider] bean name.
