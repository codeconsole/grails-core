...
The default action for the PersistentLogin controller is search. By default only the standard fields (`username`, `token`, and `series`) are available but this is customizable with the <<ui-s2ui-override>> script - see the <<ui-customization>> section for details.
