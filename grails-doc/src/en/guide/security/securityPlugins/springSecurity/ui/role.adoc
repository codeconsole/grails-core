...
The default action for the Role controller is search. By default only the `authority` field is available but this is customizable with the <<ui-s2ui-override>> script - see the <<ui-customization>> section for details.
