...
Because the check is intraprocedural, it cannot follow every possible way query text can be assembled. If a build hits a false positive with no reasonable per-call-site fix, disable the check for the whole compilation unit by setting the `protectSqlInjectionAttacks` system property to `false` (it defaults to `true`), for example via `gradle.properties`:

[source,properties]
----
systemProp.protectSqlInjectionAttacks=false
----

This is a global kill switch of last resort, not a substitute for `@SuppressWarnings("GormUnsafeQueryString")` on the specific method that needs it.
