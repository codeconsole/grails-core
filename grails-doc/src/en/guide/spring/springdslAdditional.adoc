...
A unit test's `beans` property is compiled automatically too, onto a static nested `@Configuration` class named `BeansConfiguration` that the test's application context registers; see <<unitTesting,Unit Testing>>.
