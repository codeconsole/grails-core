...
compiled — attributes assembled at runtime, a namespace no compiled tag library declares, a tag
declared by more than one of them, a tag defined as a closure, or a name something else in scope
answers to — are dispatched exactly as before. Expressions in a GSP page are checked the same way but
are not rewritten; a page still selects the tag by name as it renders, without touching a metaclass.
