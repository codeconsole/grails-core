...
keeps its runtime functionality — only its commands are unavailable until it publishes a companion
`-cli` artifact. See the xref:upgrading#upgrading80x[upgrade guide] for the migration steps and the
full upgrade-impact breakdown.
