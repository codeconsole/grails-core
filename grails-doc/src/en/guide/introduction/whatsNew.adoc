...
`grailsCliLegacy` bucket. That bucket contributes to `grailsCliClasspath`, never to application or
plugin compile classpaths. This keeps unchanged Grails 7 command plugins working by default with
zero user action without leaking `grails.dev.commands.*` into the Grails 8 command-authoring ABI.
