...
discarding it, so a datastore can act on `readOnly` and on anything else the definition carries. This is a behavior
change for MongoDB code that saves without `flush: true` and relies on a later read-only transaction to persist the
