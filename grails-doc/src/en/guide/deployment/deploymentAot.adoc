...
The task fails if what was recorded is not what was asked for: a request that answers with an error or a not-found, a page listed under `forms` that carries no form, or a named form that is not on the page. Each of those would otherwise report a successful trace having recorded the wrong page, or nothing at all.
