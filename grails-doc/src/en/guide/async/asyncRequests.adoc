...
Both `WebPromises.task { }` and promises returned from actions honor `spring.mvc.async.request-timeout` (for example, `30s`). When unset, the Servlet container's default applies (normally 30 seconds on Tomcat). This replaces the previous infinite timeout for `WebPromises`. A timeout produces HTTP 503 (Service Unavailable), without logging an application-error stack trace. An application can customize its response with a `503` URL mapping, optionally restricted to `org.springframework.web.context.request.async.AsyncRequestTimeoutException`. The timeout ends the response lifecycle; it does not cancel the underlying task. Tasks must not continue writing to the response after timeout.
