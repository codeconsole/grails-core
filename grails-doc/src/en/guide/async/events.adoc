...
When `grails.async.promiseFactory=virtual-thread` selects the opt-in factory, the event bus instead shares that factory's decorated virtual-thread executor and remains asynchronous.
