...
The Grails decorator also applies to other Boot-managed executors that consume `TaskDecorator` beans, including `@Async` execution and scheduling initiated during a web request. Avoid retaining request state in long-lived background work.

If Boot does not provide a bean named `applicationTaskExecutor`, Grails uses its managed `grailsPromiseExecutor`, with eight core threads and an unbounded queue. Other executor and scheduler beans do not change that selection. Tune the Boot executor using `spring.task.execution` for application workloads; blocking on tasks queued to the same saturated pool can deadlock. A standalone factory continues to use a cached pool, and standalone `WebPromises` propagate request context without requiring the plugin.

With the default and virtual-thread factories, direct and chained `get()` calls (including timed calls) report failures as `ExecutionException`, with the original failure as the cause, including checked exceptions such as `IOException`. Error callbacks, including list callbacks, receive the original failure, preserving its own cause chain. A single-promise `onError` can recover with its callback result; aggregate `onError` observes the failure and leaves the returned promise failed. Third-party factories retain their own contracts.
...
It uses its own executor instead of the supplied application executor. For virtual threads on the default Spring-managed path, prefer `spring.threads.virtual.enabled=true` instead.

The opt-in factory uses the same `CompletableFuture` implementation and failure semantics as the default factory. Its executor also serves the default event bus and, when registered by the async plugin, receives the application's composed `TaskDecorator` beans. It owns and closes that executor; Boot pool-size and shutdown settings do not configure it. Standalone `WebPromises` decorate the entire executor task, preserving request context through chained `then { }` callbacks as well as the initial task.
