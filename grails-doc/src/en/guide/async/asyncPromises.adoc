...
Direct and chained `get()` calls (including timed calls) report failures as `ExecutionException`, with the original failure as the cause. Error callbacks, including list callbacks, receive the original failure, preserving its own cause chain.
