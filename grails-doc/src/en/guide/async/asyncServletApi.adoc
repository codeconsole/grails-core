...
WARNING: `AsyncController` and its `startAsync()` API are deprecated for removal in Grails 8. Prefer returning a `Promise` from an action so Spring MVC manages completion, dispatch, timeouts, and errors. The examples below are retained for migrating existing applications.
