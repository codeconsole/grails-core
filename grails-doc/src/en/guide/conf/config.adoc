...
Framework modules can also register legacy Groovy configuration DSL sources for static metadata discovery. Grails
inspects the source syntax without executing the configuration script. Literal values can provide generated defaults,
while values from method calls, environment branches, and other dynamic expressions are discovered without treating a
build-specific result as the default.
