...
Other domain classes are not affected, and an operation inside the block that names a `DataSource`, such as `ZipCode.lookup.count()`, uses the one it names. A method annotated `@Transactional(connection = 'auditing')` routes in the same way, for every domain class mapped to the 'auditing' `DataSource`; a class that is not mapped to it keeps its own.
