...
We recommend https://bell-sw.com/pages/downloads/#jdk-21-lts[Liberica JDK 21 LTS] based on https://spring.io/quickstart[Spring's JDK recommendation].
...
Grails applications are built with https://gradle.org/[Gradle]. Gradle does not need to be installed separately: a generated application includes the Gradle wrapper (`gradlew`), which downloads the right Gradle version on first use. Grails {grailsMajorVersion} requires Gradle 9.7 or later and generates applications with Gradle {gradleVersion}.

To automate the installation of Grails we recommend https://sdkman.io/[SDKMAN] which greatly simplifies installing and managing multiple Grails versions.
