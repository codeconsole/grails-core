...
Each part of the name must be a valid Java identifier and none may be a keyword; anything else is a compile-time error. Name one of the plugin's own packages: generating into a package the plugin does not otherwise own splits that package across two jars, which a modular or native-image consumer pays for.
