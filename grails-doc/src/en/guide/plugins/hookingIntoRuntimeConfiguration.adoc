...
* `.annotate(...)` attribute values must be simple compile-time constants: strings, numbers, booleans, class literals, enum constants, or arrays of these. A bare constant *reference* counts — `.annotate(ConditionalOnProperty, name: Settings.WEB_URL_CONVERTER, havingValue: 'camelCase')` folds correctly, `@CompileStatic` included, because `grails.config.Settings` is an interface and its constants are real `public static final` fields. Two shapes do not fold: a `static final` declared on a Groovy *class*, which is a property with a generated getter that `@CompileStatic` rewrites the reference into a call to; and a *concatenation* of constants, which `.value(...)` does handle and `.annotate(...)` does not. An attribute that itself takes a nested annotation as its value isn't supported either.
