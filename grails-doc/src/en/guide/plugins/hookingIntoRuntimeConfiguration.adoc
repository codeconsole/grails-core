...
* Each statement inside `beans { }` must be exactly `bean(Type[, "name"]) { ... }`, optionally chained with any combination of `.conditionalOnMissingBean(Type...)`, `.primary()`, `.lazy()`, `.scope("name")`, and (repeatably) `.annotate(AnnotationType[, attr: value, ...])` — an `if`, a `for` loop, or any other imperative Groovy is a compile-time error, not a runtime one.
