...
* Those four are the only per-bean qualifiers. There is no way to attach `@ConditionalOnProperty`, `@ConditionalOnClass`, `@ConditionalOnBean`, or a custom `Condition` to a single bean — those only apply if written on the whole `AutoConfiguration` class, which then gates every bean it declares, not just one.
