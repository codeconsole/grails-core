...
* Each statement inside `beans { }` must be exactly `bean(Type[, "name"]) { ... }`, optionally chained with `.conditionalOnMissingBean(Type...)` — an `if`, a `for` loop, or any other imperative Groovy is a compile-time error, not a runtime one.
* `.conditionalOnMissingBean(Type...)` is the only per-bean qualifier. There is no `.primary()`, `.lazy()`, or `.scope()`, and no way to attach `@ConditionalOnProperty`, `@ConditionalOnClass`, `@ConditionalOnBean`, or a custom `Condition` to a single bean — those only apply if written on the whole `AutoConfiguration` class, which then gates every bean it declares, not just one.
