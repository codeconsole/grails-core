...
* Chaining `.primary()`, `.lazy()`, and/or `.scope("name")` adds `@Primary`, `@Lazy`, and `@Scope("name")` respectively. Any combination can be chained together, in any order, alongside `.conditionalOnMissingBean(...)` — for example `bean(SlowGreeter, 'slowGreeter').lazy().scope('prototype') { ... }`.
...
The DSL deliberately covers a narrower surface than `doWithSpring(BeanBuilder)`:

* Each statement inside `beans { }` must be exactly `bean(Type[, "name"]) { ... }`, optionally chained with any combination of `.conditionalOnMissingBean(Type...)`, `.primary()`, `.lazy()`, and `.scope("name")` — an `if`, a `for` loop, or any other imperative Groovy is a compile-time error, not a runtime one.
* Those four are the only per-bean qualifiers the DSL recognises today. A hand-written `@AutoConfiguration` class can freely put `@ConditionalOnProperty`, `@ConditionalOnClass`, `@ConditionalOnBean`, or a custom `Condition` directly on an individual `@Bean` method — that's standard Spring Boot practice, not something confined to the whole class — but this DSL has no chained syntax for it yet.
