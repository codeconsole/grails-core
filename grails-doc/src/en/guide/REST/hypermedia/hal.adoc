...
To return HAL instead of regular JSON for a resource you can override the renderer in
`grails-app/conf/spring/resources.groovy` with an instance of `grails.rest.render.hal.HalJsonRenderer`:
...
HAL XML renderers are deprecated since Grails 8.0 and will be removed in a future major release. Existing
clients can retain them temporarily by adding the optional `grails-xml` module. New development should
use the Grails HAL JSON renderer. Spring HATEOAS models can be returned from Spring MVC controllers;
adding `spring-boot-starter-hateoas` alone does not register a Grails renderer for those models.
