...
`springdoc.packages-to-scan` and `springdoc.packages-to-exclude` apply to the Grails endpoints of the
...
Each group takes the same `paths-to-match`, `paths-to-exclude`, `packages-to-scan` and
`packages-to-exclude` settings as the default document, and `display-name` titles it. The
...
application also has. A springdoc `GroupedOpenApi` bean the application declares is honored the same
way, both at runtime and by the command.

NOTE: Declare groups under `grails.openapi.groups` rather than springdoc's `springdoc.group-configs`.
That property is a list of objects, which springdoc cannot read from the configuration Grails loads
...
not depend on which is described first. A class sharing the name of a schema the document derives,
such as `ValidationErrors` or a `Patch` schema, is named by its package in the same way.
