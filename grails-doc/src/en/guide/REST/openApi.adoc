...
documented for each action the controller declares, and one that chooses the action by the method
of the request, such as `action: [GET: 'show', DELETE: 'delete']`, for each method. A mapping whose
...
not part of it. The constraints, and whether it is required or read only, follow a
renamed property. Grails' converters, JSON views and data binding use the property's own name, so a
Jackson rename describes a type the application renders and binds with Jackson. A property of a
domain class or a command object is otherwise described by the name Grails uses, even where Jackson
would write it another way, such as `ISBN`, which Jackson writes as `isbn`, and a command object's
...
its package in the same way. An enum is described where it is used, and so shares no name, unless
...
version is not rendered either unless `grails.converters.domain.include.version` is set, and is
described only then.
...
* A controller with an errors view of its own, such as `grails-app/views/book/_errors.gson`, is
  described answering in JSON without a shape, since only the application knows it.
...
command object as an element named for its class, such as `book`, its identifier and version as
attributes, an association as an element carrying the associated identifier as an attribute, and a
listing as a `list` element holding one for each.
...
`responseFormats`. The first form above produces `GET /book`, `POST /book` and `GET /book/{id}`; the
...
A controller rendering views for a browser declares no `responseFormats`, so it does not appear in the
