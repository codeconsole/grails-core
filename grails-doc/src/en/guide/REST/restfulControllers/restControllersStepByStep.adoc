...
The key to implementing REST actions is the link:{controllersRef}respond.html[respond] method introduced in Grails 2.3. The `respond` method tries to produce the most appropriate response for the requested content type (JSON, XML, HTML etc.)
