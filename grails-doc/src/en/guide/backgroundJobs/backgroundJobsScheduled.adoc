...
Nothing in `@Scheduled` is aware of other instances of your application. Each instance builds its own scheduler from the annotations it finds and fires on its own timer. With three instances deployed, a `@Scheduled(cron = '0 0 6 * * ?')` method runs three times at 6:00am — concurrently, on three machines, against the same database.

For anything that must run exactly once per schedule across a deployment, see <<backgroundJobsClustering,Clustered Jobs>>.
