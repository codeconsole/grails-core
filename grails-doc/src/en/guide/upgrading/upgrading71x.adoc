...
==== 3. Precompiled GSPs Record a Checksum Instead of a Modification Time
...
Pages compiled by Grails 7.1.6 and later record a `SOURCE_CHECKSUM` of the page source instead, and
...
by Grails 7.1.6 or later, since no modification time is recorded for them. Use `getSourceChecksum()` to
...
checksum. An application still running an earlier Grails that consumes a plugin precompiled by 7.1.6 or later
