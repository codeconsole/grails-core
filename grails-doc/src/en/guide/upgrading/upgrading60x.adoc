...
As of Grails 7, the Quartz plugin is part of Grails core and is documented in the xref:backgroundJobs.adoc#backgroundJobsClustered[Background Jobs > Clustered Scheduling] section of this guide.
...
The plugin builds its scheduler from `SchedulerFactoryBean`, which lives in `org.springframework:spring-context-support`. Plugin version 4.0.x did not declare that dependency, and it is not reliably inherited from the rest of the stack, so an application could fail at startup with a `NoClassDefFoundError` until `spring-context-support` was added by hand.
