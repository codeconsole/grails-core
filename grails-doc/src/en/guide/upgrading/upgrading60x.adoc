...
==== 18. Interceptor URI Matching Uses the Dispatched Path
...
The `uri` argument passed to `doesMatch` is now the decoded, application-relative path without matrix parameters.

The Spring Security plugin applies the same canonicalization when matching `ipRestrictions` patterns, so an encoded or
matrix-parameter variant of a restricted path is subject to the same restriction as the plain path.
...
matched. Interceptors and the Spring Security plugin follow dispatch on both points because a matcher that disagrees
with dispatch lets a request reach a controller without its interceptors or restrictions.
