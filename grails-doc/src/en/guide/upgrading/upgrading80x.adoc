...
==== 34. Logback Configuration No Longer Generated by Default

Newly generated Grails 8 applications no longer include a `grails-app/conf/logback-spring.xml` file.

Logging still works out of the box. A generated application depends on `grails-logging`, which brings in Spring Boot's logging starter, so Logback and Spring Boot's own default Logback configuration are on the classpath and apply automatically when the application supplies no configuration of its own. This matches the Spring Boot convention of shipping no logging configuration file unless one is actually needed.
...
Reach for a `logback-spring.xml` file only when properties are not enough — for example custom appenders, structured/JSON output, or per-environment appender routing. Use the `-spring` variant (not plain `logback.xml`), because it is processed by Spring Boot and unlocks `<springProfile>` and `<springProperty>`:
...
TIP: When generating a project with https://start.grails.org[Grails Forge], select the *Logback Configuration* feature to have a starter `grails-app/conf/logback-spring.xml` generated for you.
