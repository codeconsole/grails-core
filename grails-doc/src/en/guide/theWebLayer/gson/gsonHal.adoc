...
The default HAL response content type is `application/hal+json`, however as discussed in the section on <<mimeTypes,custom MIME types>> you can define your own response content types to represent your resources.
...
json {
    hal.paginate(Book, bookCount, offset, max, sort, order)
    ...
}
