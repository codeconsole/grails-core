...
NOTE: The controller is resolved from the domain class the controller exposes, so a controller does
not have to be named after its domain class. A `PeopleController` declared as
`RestfulController<Person>` is the target of a `resource` link for a `Person` instance. If more than
one controller exposes the same domain class, or none does, the link falls back to the domain class
name; pass the `controller` attribute explicitly in that case.
