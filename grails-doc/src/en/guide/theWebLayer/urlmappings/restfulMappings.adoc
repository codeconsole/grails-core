...
`namespace` attribute when one is given, as it is for every redirect from a controller, and otherwise the
namespace of the current request, which outside a request is the default namespace. When no scope
settles on a single controller, the controller named after the domain class is assumed.
