...
the link targets
. the only controller in that namespace serving the domain class
. the controller named after the domain class
. the only controller declaring the domain class

The namespace a link targets is its `namespace` attribute when one is given, as it is for every redirect
from a controller, and otherwise the namespace of the current request. A level with more than one
candidate falls through to the next, and when nothing matches, the controller named after the domain
class is assumed.
