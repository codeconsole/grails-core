...
Grails 8 binds request data only to allowlisted properties by default.  Existing link:{constraintsRef}bindable.html[`bindable: true`] declarations, explicit `include` lists, and `grails.web.databinding.BindAllowed` controller action parameter annotations continue to work unchanged and bind exactly the properties they name.  A class that relied on permissive binding without an allowlist now binds no request properties.  Add `bindable: true`, an explicit `include` list, or `@BindAllowed` for each intended property.  Applications may set `grails.databinding.legacyBindableDefault=true` to restore permissive legacy binding for the whole application.
