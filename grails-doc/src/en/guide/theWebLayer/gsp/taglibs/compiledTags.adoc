...
A tag library referring to something the same project declares — a service it injects, a base class it
extends, a trait it carries — is described too: the task reads that source alongside it, so an
inherited namespace, a tag a trait contributes and an attributes parameter of a project-declared type
are all read rather than guessed. A tag library naming a type that does not exist is left out, as it
