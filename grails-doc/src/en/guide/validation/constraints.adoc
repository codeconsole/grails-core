...
Within a domain class link:{domainClassesRef}constraints.html[constraints] are defined with the constraints property that is assigned a code block:
...
See how the `inList` constraint references the instance property `survey`? That won't work. Instead, use a custom link:{constraintsRef}validator.html[validator]:
