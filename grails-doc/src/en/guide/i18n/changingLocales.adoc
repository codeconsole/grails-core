...
`session` and `cookie` are mutable, so the `?lang=` switch works with them. `acceptHeader` (the locale follows the incoming `Accept-Language` header) and `fixed` are read-only, so the `?lang=` switch has no effect and its interceptor is not registered. The `fixed` resolver uses `grails.i18n.default.locale`, falling back to the JVM default locale.
