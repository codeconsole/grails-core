...
The primary command for creating a new project is `create-app`, which creates a standard Grails web application that communicates over HTTP.
For other types of applications, see the documentation below.

Creating a project is the primary usage of the Grails Forge CLI.
You can also generate an application with grails-shell, which uses profiles for generation.
