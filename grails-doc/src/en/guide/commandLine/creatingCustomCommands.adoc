...
2. **Define the Command in a `cli` source set:** plugin commands live in `src/cli/groovy` (not
`grails-app/commands`, which is compiled into the runtime jar). Give them a package that is unique
to the cli artifact:
+
[source,groovy]
----
