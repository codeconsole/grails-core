...
class UndertowWebServerFactoryCustomizerTests {
    ...
    @Test
    void defaultMaxHttpPostSizeIs2MB() {
        // 2MB, matching Undertow 2.4's hardened DEFAULT_MAX_ENTITY_SIZE
        // (CVE-2024-3884, CVE-2024-4027)
        bind();
        assertThat(boundServerOption(UndertowOptions.MAX_ENTITY_SIZE))
            .isEqualTo(UndertowOptions.DEFAULT_MAX_ENTITY_SIZE);
    }

    @Test
    void negativeMaxHttpPostSizeMapsToUnlimited() {
        bind("server.undertow.max-http-post-size=-1");
        assertThat(boundServerOption(UndertowOptions.MAX_ENTITY_SIZE)).isEqualTo(-1L);
    }
